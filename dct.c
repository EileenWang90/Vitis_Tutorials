...
void read_data(short* input, short buf[DCT_SIZE][DCT_SIZE])
{
   int r, c;
...
void write_data(short buf[DCT_SIZE][DCT_SIZE], short* output)
{
   int r, c;
...
void dct (//(short input[N], short output[N]){
      const short *in1, // Read-Only Vector 1
      short *out,            // Output Result
      int size )            // Size in integer
{
   ...
   read_data(in1, buf_2d_in);

   dct_2d(buf_2d_in, buf_2d_out);

   // Write out the results.
   write_data(buf_2d_out, out);
}
